      ...
      def build_http_metrics(http_totals, http_durations)
        ...
        http_durations.sort.each do |(method, path, status), durations|
          HTTP_BUCKETS.each do |bucket|
            ...
            lines << %(fiscalbridge_http_request_duration_seconds_bucket{method="#{method}",path="#{path}",status="#{status}",le="#{bucket}"} #{count})
          end

          lines << %(fiscalbridge_http_request_duration_seconds_bucket{method="#{method}",path="#{path}",status="#{status}",le="+Inf"} #{durations.count})
          lines << %(fiscalbridge_http_request_duration_seconds_sum{method="#{method}",path="#{path}",status="#{status}"} #{durations.sum.round(6)})
          lines << %(fiscalbridge_http_request_duration_seconds_count{method="#{method}",path="#{path}",status="#{status}"} #{durations.count})
        end
